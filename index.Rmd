---
title: "Climate Migration: first look data & trends"
author: "Parth Khare"
...
# Long Temp Change 
clidel12ann_grd$Tmpdf.2015_1900 <- clidel12ann_grd$Temp_2015 - clidel12ann_grd$Temp_1900
# Short terms increase
clidel12ann_grd$Tmpdf.2015_1970 <- clidel12ann_grd$Temp_2015 - clidel12ann_grd$Temp_1970
...
gmp1 <- ggplot() + annotation_map_tile("cartolight") +
  geom_sf(data = clidel12ann_grd, aes(fill = X201015), lwd=0) +
  theme_void() + theme(legend.position="bottom") + ggtitle("Net Migraiton 2010-2015") +
...
gmp2 <- ggplot() + annotation_map_tile("cartolight") +
  geom_sf(data = clidel12ann_grd, aes(fill = Tmpdf.2015_1900), lwd=0) +
  theme_void() + theme(legend.position="bottom") + ggtitle("Long term temperature change (2015-1900") +
  scale_fill_viridis_c(breaks = quantile(clidel12ann_grd$Tmpdf.2015_1900, seq(0,1,0.2)),
...
gmp3 <- ggplot() + annotation_map_tile("cartolight") +
  geom_sf(data = clidel12ann_grd, aes(fill = Tmpdf.2015_1970), lwd=0) +
  theme_void() + theme(legend.position="bottom") + ggtitle("Shorter term temperature change (2015-1970") +
  scale_fill_viridis_c(breaks = quantile(clidel12ann_grd$Tmpdf.2015_1970, seq(0,1,0.2)),
                       option = "inferno",trans = scales::pseudo_log_trans(sigma = 0.001))
...
### 101: Do extreme temperature change correspond with high emigration?
...
lowbound= -3; hghbound=3
ggplot(clidel12ann_grd %>% filter(Tmpdf.2015_1900 <= lowbound | Tmpdf.2015_1900 >= hghbound), 
  ...
  geom_smooth(data= clidel12ann_grd %>% filter(Tmpdf.2015_1900 <= lowbound), size=0.5,se=F,col='blue') + 
  geom_smooth(data= clidel12ann_grd %>% filter(Tmpdf.2015_1900 >= hghbound), size=0.5,se=F,col='red') + 
...
# Temperature rise and migration II: grids with extreme change in temp
lowbound= -3; hghbound=3  
ggplot(clidel12ann_grd %>% filter(Tmpdf.2015_1970 <= lowbound | Tmpdf.2015_1970 >= hghbound), 
  # aes(x=Tmpdf.2015_1970,y=log(X201015*(-1)))) + geom_point() + 
  aes(x=Tmpdf.2015_1970,y=log(abs(X201015)))) + geom_point() + 
  ylab('Net migration') + xlab('Temperature Change') +
  geom_vline(xintercept=0, linetype='dotted', color='grey40', size=0.5) +
  geom_hline(yintercept=0, linetype='dotted', color='grey40', size=0.5) +
  geom_smooth(data= clidel12ann_grd %>% filter(Tmpdf.2015_1970 <= lowbound), size=0.5,se=F,col='blue') + 
  geom_smooth(data= clidel12ann_grd %>% filter(Tmpdf.2015_1970 >= hghbound), size=0.5,se=F,col='red') + 
  theme_minimal() + ggtitle("95%ile grids: Temperature rise (1900 to 1975) vs Migration 1975-80")
```
